...
def process_files_into_session_state(selected_files: List[Path]):
    ...
    if selected_files:
        try:
            oldest_timestamp = min(os.path.getmtime(file) for file in selected_files)
            # Adjust for server time if not local, assuming server is in a different timezone
            is_local = st.get_option('server.address') in ('localhost', '127.0.0.1', None)
            if not is_local:
                oldest_timestamp -= (4 * 3600)  # Adjust for timezone difference, e.g., 4 hours for EDT
            st.session_state.csv_timestamp = time.strftime("%b %d, %Y %I:%M %p", time.localtime(oldest_timestamp))
        except Exception as e:
            st.session_state.csv_timestamp = "Timestamp unavailable"
            st.warning(f"Could not determine file timestamp: {e}")

...
from modules.schedule_analysis.playoff_odds_ui import display_playoff_odds
...
def display_team_stats(schedule_df, calculate_team_stats_fn):
    """Display team performance statistics."""
    team_stats = calculate_team_stats_fn(schedule_df)
...
def display_swap_selection(all_teams, schedule_df):
    ...
    if "schedule_swap_results" in st.session_state:
        ...
        plotly_chart(
            create_win_change_barchart(results["comparison"], results["team1"], results["team2"]),
            width="stretch",
        )
...
def display_all_swaps_analysis(all_teams, all_swaps_df, schedule_df):
    ...
    filtered_df = (
        ...
        else all_swaps_df[(all_swaps_df["Team 1"] == selected_team) | (all_swaps_df["Team 2"] == selected_team)].copy()
    )
    ...
    if selected_team == "All Teams":
        ...
        filtered_df = filtered_df.sort_values(by=sort_keys, ascending=[False] + [False] * (len(sort_keys) - 1))
    ...
    def style_change(val):
        return (
            ...
            if val > 0
            else ("color: #dc3545; font-weight: bold;" if val < 0 else "color: #6c757d;")
        )
    ...
    if "Selected Team Change" in filtered_df.columns:
        display_columns.insert(2, "Selected Team Change")
    ...
    dataframe(
        ...
        column_config={
            ...
            "Selected Team Change": st.column_config.NumberColumn("Selected Team ΔPos"),
            "Team 1 Position Change": st.column_config.NumberColumn("T1 Change"),
            "Team 2 Position Change": st.column_config.NumberColumn("T2 Change"),
            "Biggest Winner": st.column_config.TextColumn("Biggest Winner", width="medium"),
            "Winner Change": st.column_config.NumberColumn("Winner Change"),
            "Biggest Loser": st.column_config.TextColumn("Biggest Loser", width="medium"),
            "Loser Change": st.column_config.NumberColumn("Loser Change"),
        },
    )
...
def display_current_period_overview(schedule_df, current_period: int | None):
    ...
    scoring_label = (
        str(period_df["Scoring Period"].iloc[0]) if hasattr(period_df["Scoring Period"], "iloc") else None
    )
